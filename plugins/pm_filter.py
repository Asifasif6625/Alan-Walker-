...
async def pm_next_page(bot, query):
    ...
    if n_offset == 0:
        ...
        btn.append(
            [InlineKeyboardButton(f"℘ {math.ceil(int(offset) / 10) + 1} / {math.ceil(total / 10)} ℘", callback_data="pages"),
             InlineKeyboardButton("ｎｅｘｔ❡", callback_data=f"pmnext_{req}_{key}_{n_offset}")])
    else:
        btn.append(
            [
                InlineKeyboardButton("❡ｂａｃｋ", callback_data=f"pmnext_{req}_{key}_{off_set}"),
...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("❡ｂａｃｋ", callback_data=f"next_{req}_{key}_{off_set}"),
        ...
        btn.append(
            [InlineKeyboardButton(f"℘ {math.ceil(int(offset) / 10) + 1} / {math.ceil(total / 10)} ℘", callback_data="pages"),
             InlineKeyboardButton("ｎｅｘｔ❡", callback_data=f"next_{req}_{key}_{n_offset}")])
    else:
        btn.append(
            [
                InlineKeyboardButton("❡ｂａｃｋ", callback_data=f"next_{req}_{key}_{off_set}"),
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):        
        ...
        buttons = [[
            ...
            InlineKeyboardButton('☣️Auto Filter', callback_data='autofilter'),
            ...
            InlineKeyboardButton('🧺Files Store', callback_data='newdata')
            ],[           
            InlineKeyboardButton('🛡️Jsone', callback_data='son'),
            InlineKeyboardButton('🎤Tts', callback_data='ttss'),           
            InlineKeyboardButton('🛒Purge', callback_data='purges')
            ],[
            InlineKeyboardButton('🧴Pastes', callback_data='pastes'),
            InlineKeyboardButton("🗺️Photos Editor", callback_data='image'),
            ...
            InlineKeyboardButton('🖍️Carbon', callback_data='carb'),
            InlineKeyboardButton('📝Fonds', callback_data='fond'),
            InlineKeyboardButton('🧟‍♂️Zombie', callback_data='zombies')
            ],[
            InlineKeyboardButton('🗃️Data Storege', callback_data='stats')
            ],[
            InlineKeyboardButton('🧹Close', callback_data='close_data'),
            InlineKeyboardButton('🏕️back Home', callback_data='start')           
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)             
        await query.message.edit_text(                     
        ...
        buttons = [[
            InlineKeyboardButton('⭕️𝚄𝙿𝙳𝙰𝚃𝙴 𝙲𝙷𝙰𝙽𝙽𝙴𝙻⭕️', url='https://t.me/mlavib')
            ],[
            InlineKeyboardButton('🏕️Go Home', callback_data='start')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            InlineKeyboardButton('🚸Go Back', callback_data='manuelfilter')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            InlineKeyboardButton('⚙️Admin Portal', callback_data='admin')
            ],[
            InlineKeyboardButton('🚸Go Back', callback_data='help'),
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        ...
        if query.from_user.id in ADMINS:
            ...
            await query.answer("അയ്യോ... 🙄നിങ്ങൾക്ക് അഡ്മിൻ പാനൽ ഓപ്പൺ ആക്കാൻ കഴിയില്ല", show_alert=True)

    elif query.data == "gfill":
        buttons = [[            
            InlineKeyboardButton('🚸Go Back', callback_data='admin')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)        
...
async def auto_filter(client, msg, spoll=False):
    if not spoll:
        ...
        if 2 < len(message.text) < 100:
            ...
            if not files:
                z = await message.reply_text(text=f"<b>അതേ നിങ്ങൾ അയച്ച സ്പെല്ലിങ് തെറ്റ് ആണ്. ഗൂഗിൾ നോക്കി ഒന്നും കൂടി മൂവി നെയിം അയക്കുക.\nനിങ്ങൾ ചോദിച്ച മൂവി OTT റിലീസ് ആയെങ്കിൽ ഞാൻ ഫയൽ തരും.\n\n𝘠𝘦𝘴, 𝘵𝘩𝘦 𝘴𝘱𝘦𝘭𝘭𝘪𝘯𝘨 𝘺𝘰𝘶 𝘴𝘦𝘯𝘵 𝘪𝘴 𝘪𝘯𝘤𝘰𝘳𝘳𝘦𝘤𝘵. 𝘑𝘶𝘴𝘵 𝘴𝘦𝘢𝘳𝘤𝘩 𝘎𝘰𝘰𝘨𝘭𝘦 𝘢𝘯𝘥 𝘴𝘦𝘯𝘥 𝘵𝘩𝘦 𝘮𝘰𝘷𝘪𝘦 𝘯𝘢𝘮𝘦.\n𝘐𝘧 𝘵𝘩𝘦 𝘮𝘰𝘷𝘪𝘦 𝘺𝘰𝘶 𝘢𝘴𝘬𝘦𝘥 𝘧𝘰𝘳 𝘪𝘴 𝘢𝘯 𝘖𝘛𝘛 𝘳𝘦𝘭𝘦𝘢𝘴𝘦, 𝘐 𝘸𝘪𝘭𝘭 𝘱𝘳𝘰𝘷𝘪𝘥𝘦 𝘵𝘩𝘦 𝘧𝘪𝘭𝘦.</b>")
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"℘ 1/{math.ceil(int(total_results) / 6)} ℘", callback_data="pages"),
             InlineKeyboardButton(text="ｎｅｘｔ❡", callback_data=f"next_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
            [InlineKeyboardButton(text="℘ -ｎｏ ｍｏｒｅ ｐａｇｅ- ℘", callback_data="pages")]
        )
    ...
    if imdb:
        ...
        cap = f"<b>ტ Title :- {search}</b>\n\n<b>ტ Requested By :- {message.from_user.mention}</b>\n\n<b>ტ {message.chat.title} </b>\n\n<b> ტ ｃｌｉｃｋ ｔｈｅ ｂｕｔｔｏｎ ａｎｄ ｄｏｗｎｌｏａｄ ｍｏｖｉｅ </b>"
